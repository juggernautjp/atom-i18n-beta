...
class I18N {
  constructor() {
    ...
    if (!atom.config.get('atom-i18n-beta.customMenuI18nPath')) {
      atom.config.set(
        'atom-i18n-beta.customMenuI18nPath',
        path.join(__dirname, '../def', 'custom_menu.cson'),
      );
    }
    const LOCALE = atom.config.get('atom-i18n-beta.locale');
    this.defM = CSON.load(
  ...
  activate() {
    ...
    atom.commands.add(
      'atom-workspace',
      'atom-i18n-beta:open-custom-menu-i18n-file',
      () => {
        atom.workspace.open(atom.config.get('atom-i18n-beta.customMenuI18nPath'));
      },
    );
  }
  ...
  customMenuI18n() {
    const customMenuCson = atom.config.get('atom-i18n-beta.customMenuI18nPath');
