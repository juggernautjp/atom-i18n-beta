...
module.exports = class Consent {
  ...
  static updateConsent(pane) {
    if (pane) {
      // console.log("updateConsent(pane): ", pane);
    } else {
      // console.log("updateConsent(pane): ", "nill");
    }
    ...
    */
    ...
    consentTab.querySelector('.title').textContent = this.defO.Consent.tabTitle;
    ...
    if (consent.getAttribute('data-localized') !== 'true') {
      ...
      if (desc !== null) {
        let {text1, text2, text3} = this.defO.Consent.desc;
      ...
      if (chioceGroup !== null) {
        ...
        btnsNodeList[0].textContent = this.defO.Consent.send['send-usage'];
        btnsNodeList[1].textContent = this.defO.Consent.unsend['unsend-usage'];
        // Line #3: Notes
        const noteNodeList = chioceGroup.querySelectorAll('p.welcome-note');
        let node = noteNodeList[0];
        if (node !== null) {
          const {text1, text2, text3} = this.defO.Consent.send.notes;
        ...
        node = noteNodeList[1];
        if (node !== null) {
          node.textContent = this.defO.Consent.unsend.notes;
        }
      }
      // Line #4: with Love by
      const {with: _with, by} = this.defO.Consent;
